...
When updating this object, only the [Status](#status) element can be set if you want to approve, decline, or cancel an insertion order before it becomes approved. You can only approve, decline, or cancel via this object if the [Status](#status) is set to PendingUserReview. Once the insertion order [Status](#status) is Active, Exhausted, Expired, or NotStarted, then you can either make new changes or approve, decline, or cancel the current pending changes via the [PendingChanges](#pendingchanges) element. If the [Status](#status) is Canceled or Declined then you cannot update the insertion order. 
...
|<a name="pendingchanges"></a>PendingChanges|Can be used to manage changes for an approved insertion order with status set to either Active, Exhausted, Expired, or NotStarted.|[InsertionOrderPendingChanges](insertionorderpendingchanges.md)|

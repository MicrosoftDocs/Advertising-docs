...
When you call a service operation such as [GetCampaignsByAccountId](/binga/bingads/campaign-management-service/getcampaignsbyaccountid.md), you must specify [request header](#request-headers) elements such as DeveloperToken, CustomerId, and CustomerAccountId.
...
Call [GetUser](/binga/bingads/customer-management-service/getuser.md) with your Bing Ads credentials and DeveloperToken. Within the Body set the UserId nil. The response will include a [User](/binga/bingads/customer-management-service/user.md) object that contains the UserId.
...
Then call [SearchAccounts](/binga/bingads/customer-management-service/getuser.md) with the UserId returned via the previous step. The returned advertiser account (or accounts) will include account and customer identifiers.

...
```csharp
protected async Task<AddConversionGoalsResponse> AddConversionGoalsAsync(
	IList<ConversionGoal> conversionGoals)
{
	...
	return (await CampaignManagement.CallAsync((s, r) => s.AddConversionGoalsAsync(r), request));
}
```
```java
static AddConversionGoalsResponse addConversionGoals(
	ArrayOfConversionGoal conversionGoals)
{
	AddConversionGoalsRequest request = new AddConversionGoalsRequest();

	request.setConversionGoals(conversionGoals);

	return CampaignManagement.getService().addConversionGoals(request);
}
```
```php

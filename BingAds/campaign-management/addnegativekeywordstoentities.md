...
```csharp
protected async Task<AddNegativeKeywordsToEntitiesResponse> AddNegativeKeywordsToEntitiesAsync(
	IList<EntityNegativeKeyword> entityNegativeKeywords)
{
	...
	return (await CampaignManagement.CallAsync((s, r) => s.AddNegativeKeywordsToEntitiesAsync(r), request));
}
```
```java
static AddNegativeKeywordsToEntitiesResponse addNegativeKeywordsToEntities(
	ArrayOfEntityNegativeKeyword entityNegativeKeywords)
{
	...
	return CampaignManagement.getService().addNegativeKeywordsToEntities(request);
}
```
```php

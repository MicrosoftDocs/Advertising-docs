...
|-----------|---------------|-------------|
|<a name="conversiongoals"></a>ConversionGoals|An array of [ConversionGoal](../campaign-management/conversiongoal.md) objects to update within the account's shared conversion goal library.<br /><br />You can update a maximum of 100 conversion goals in a single call. <br/><br/>The account is determined by the required *CustomerAccountId* header element.|[ConversionGoal](conversiongoal.md) array|
...
```csharp
protected async Task<UpdateConversionGoalsResponse> UpdateConversionGoalsAsync(
	IList<ConversionGoal> conversionGoals)
{
	...
	return (await CampaignManagement.CallAsync((s, r) => s.UpdateConversionGoalsAsync(r), request));
}
```
```java
	...
	ArrayOfConversionGoal conversionGoals)
{
	...
	return CampaignManagement.getService().updateConversionGoals(request);
}
```
```php

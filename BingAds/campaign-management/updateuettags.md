...
|-----------|---------------|-------------|
|<a name="uettags"></a>UetTags|An array of [UetTag](../campaign-management/uettag.md) objects to update within the customer's shared UET tag library.<br /><br />You can update a maximum of 100 UET tags in a single call.|[UetTag](uettag.md) array|
	...
	IList<UetTag> uetTags)
{
	...
	return (await CampaignManagement.CallAsync((s, r) => s.UpdateUetTagsAsync(r), request));
}
```
```java
static UpdateUetTagsResponse updateUetTags(
	ArrayOfUetTag uetTags)
{
	UpdateUetTagsRequest request = new UpdateUetTagsRequest();

	request.setUetTags(uetTags);

	return CampaignManagement.getService().updateUetTags(request);
}
```
```php
static function UpdateUetTags(
	...
	$updateUetTagsRequest = new UpdateUetTagsRequest();

	$request->UetTags = $uetTags;

	return $CampaignManagementProxy->GetService()->UpdateUetTags($request);
}
```
```python
response=campaignmanagement.UpdateUetTags(
	UetTags=UetTagsHere
)
```

	...
	IList<UetTag> uetTags)
{
	...
	return (await CampaignManagement.CallAsync((s, r) => s.AddUetTagsAsync(r), request));
}
```
```java
	...
	ArrayOfUetTag uetTags)
{
	...
	return CampaignManagement.getService().addUetTags(request);
}
```
```php

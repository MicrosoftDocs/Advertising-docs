...
```csharp
protected async Task<GetSharedEntitiesByAccountIdResponse> GetSharedEntitiesByAccountIdAsync(
	string sharedEntityType)
{
	...
	return (await CampaignManagement.CallAsync((s, r) => s.GetSharedEntitiesByAccountIdAsync(r), request));
}
```
```java
static GetSharedEntitiesByAccountIdResponse getSharedEntitiesByAccountId(
	string sharedEntityType) throws RemoteException, Exception
{
	...
	return CampaignManagement.getService().getSharedEntitiesByAccountId(request);
}
```
```php

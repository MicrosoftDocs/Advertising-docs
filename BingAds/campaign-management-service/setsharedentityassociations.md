	...
	IList<SharedEntityAssociation> associations)
{
	...
	return (await CampaignManagement.CallAsync((s, r) => s.SetSharedEntityAssociationsAsync(r), request));
}
```
```java
	...
	ArrayOfSharedEntityAssociation associations) throws RemoteException, Exception
{
	...
	return CampaignManagement.getService().setSharedEntityAssociations(request);
}
```
```php

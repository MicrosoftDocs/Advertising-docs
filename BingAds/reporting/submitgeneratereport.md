...
> [!NOTE]
> You must use the same user credentials for the [SubmitGenerateReport](../reporting/submitgeneratereport.md) and [PollGenerateReport](../reporting/pollgeneratereport.md).
...
|-----------|---------------|-------------|
|<a name="reportrequest"></a>ReportRequest|The report request. The request must be an object that derives from [ReportRequest](../reporting/reportrequest.md). For a list of report request types, see [Report Types](~/guides/report-types.md).|[ReportRequest](reportrequest.md)|
...
|Element|Description|Data Type|
|-----------|---------------|-------------|
|<a name="reportrequestid"></a>ReportRequestId|The identifier of the report request. Use this identifier when calling the [PollGenerateReport](../reporting/pollgeneratereport.md) to determine the status of the report request. Once returned, the identifier is valid for two days.<br /><br />The *string* has a length up to 40 and can contain any character.|**string**|

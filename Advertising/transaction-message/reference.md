...
|Element|Description|Children
|-|-|-
|PropertyDataSet|Optional.<br /><br />A container object that contains the metadata for rooms and packages. Use `RoomData` to describe a type of room and `PackageData` to describe a room bundle. The difference between the room data and package data is that the package data also includes amenities and terms that aren't part of the physical room description. For rooms that aren't part of a package, just use `RoomData`.|[PropertyDataSet Type](#propertydatasettype)
...
|PackageData|Describes a room bundle. Package data is similar to room data except it also includes amenities and terms that are part of a package deal.|[PackageData Type](#packagedatatype)
...
|RoomBundle|Optional.<br /><br /> |Array of [roomPriceDataType](#roompricedatatype)
...
Defines a room bundle, which describes a room and any amenities and terms that are part of the package deal.

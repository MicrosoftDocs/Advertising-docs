...
|<a name="time"></a>Time|The time period to use for the report. You can specify a custom date range or select a predefined date range, for example, Today or ThisMonth.<br/><br/>The budget summary report is always aggregated by day.<br/><br/>You can set the time zone within the [BudgetSummaryReportTime](budgetsummaryreporttime.md) object, which helps you accurately scope data for the requested time period.<br/><br/>If you do not choose a time zone, the Reporting service uses PacificTimeUSCanadaTijuana by default.|[BudgetSummaryReportTime](budgetsummaryreporttime.md)|

...
|```Custom parameters```|Include up to 3 key and value pairs, which automatically fill up in the click URL.List the key and value pairs within braces. Each term should be set in quotation marks. Separate each phrase of the pair with a colon and separate key and value pairs with commas and without any spaces.Neither one can exceed 16 characters or 200 bytes.>This will not be shown in the ad.|```{"Bstyle":"SUV",“state”:”new”,”condition”:”excellent”}```|

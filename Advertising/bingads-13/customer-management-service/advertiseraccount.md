...
|<a name="paymentmethodid"></a>PaymentMethodId|The identifier of the payment instrument to use to settle the account.<br/><br/>**Add:** Optional for [AddAccount](addaccount.md); Read-only for [SignupCustomer](signupcustomer.md). When you call [SignupCustomer](signupcustomer.md) set this element to NULL. The service picks up the payment method identifier associated with the aggregator customer's invoice automatically.<br/>**Update:** Optional. If no value is set for the update, this setting is not changed.|**long**|
...
|<a name="primaryuserid"></a>PrimaryUserId|The identifier of the account manager who is primarily responsible for managing this account.<br/><br/>Only Super Admin and Standard users can be set as the primary contact for an account.<br/><br/>When you call [SignupCustomer](signupcustomer.md) the value is set to the aggregator's user identifier.<br/><br/>**Add:** Read-only<br/>**Update:** Optional. If no value is set for the update, this setting is not changed.|**long**|
